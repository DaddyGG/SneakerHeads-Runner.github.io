...
export default class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.player = this.add.sprite(200, this.height - 125, 'run');
    this.player.anims.play('run');

    const title = this.make.text({
      x: this.width / 2,
      y: this.height / 2 - 140,
      text: 'The official Sneaker Heads game',
      style: {
        fontSize: '55px',
        fill: '#ffffff',
    ...
    this.exitBtn = this.add.image(this.width / 2, this.height / 2 + 100, 'highscore').setInteractive({ useHandCursor: true }).setOrigin(0.5, 0.5)
  ...
  exitIsPressed() {
    this.exitBtn.setTexture('highscorePressed');
  }

  exitNotPressed() {
    this.exitBtn.setTexture('highscore');
  }
}